...
| Fault Tolerance | `CircuitBreaker` / `RouteCircuitBreaker` (generation-bound half-open permits), `WorkerBulkhead`, `LoadShedder` / `OnlineLoadShedder`, `ChannelHealthMonitor` backoff, `FaultInjector`, fail-open Redis + **bounded** fail-open rate limiter, explicit recall degradation (`X-Recall-Degradation-Reason`), graceful drain, multi-region DR — see [Fault Tolerance investigation](docs/system_design/18_Fault_Tolerance.md) |
...
shedding and admission control, channel-health backoff, fail-open Redis and
bounded fail-open rate limiting, graceful drain, and multi-AZ / multi-region
survival — is covered in
