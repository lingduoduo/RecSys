...
/**
 * The {@code System.gc()}-dependent half of {@link JvmMetricsBinder}'s tests. Split out of
 * {@link JvmMetricsBinderTest} specifically so that class can run in the {@code resilience}
 * profile: this one waits on real, provoked GC and polls for an asynchronously-delivered JMX
 * notification, which is exactly the kind of timing dependency issue #261 burned that profile
 * on before. Mirrors the {@link com.recsys.jvm.GcEventTrackerTest} /
 * {@link com.recsys.jvm.GcEventTrackerLifecycleTest} split.
 */
class JvmMetricsBinderGcObservationTest {
     ...
     * {@code JvmMetricsBinderTest#secondBindOnSameRegistryDoesNotRetainAnotherGcMetrics()}
     * already pins — that a second bind on the same registry does not install a second JMX
     * listener. This test is kept anyway because "meters keep reporting after real GC pressure"
     * is still a true, worthwhile regression guard; it is just not evidence for {@code RETAINED}
     * specifically.
     */
    @Test
